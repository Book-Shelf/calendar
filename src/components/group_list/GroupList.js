...
export default function GroupList(props) {
    const [checked, setChecked] = React.useState(Array.from(props.groupInstances));
    ...
    return (
        <div style={{overflow: 'auto', height: `Calc(100vh - 48px)`, width: 'auto', float: 'left', backgroundColor: '#C71F28'}}>
            {checked.map((groupInstance, index) => {
                return <GroupPlaque key={index} groupInstance={groupInstance} onChange={() => handleChange(index)} checked={checked[index]} />
            })}
        </div>
    )
}
